...
This file contains the NLSE propagator class for homogeneous wavepackets.
...
from numpy import dot, eye, atleast_2d, zeros_like, complexfloating
...
__all__ = ["NLSEPropagator"]


class NLSEPropagator(Propagator):
    ...
    def __init__(self, parameters, potential, packets=[]):
        ...
        self._kappa = self._parameters["nl_kappa"]
        self._alpha = self._parameters["nl_alpha"]
    ...
    def propagate(self):
        ...
        for packet, leading_chi in self._packets:
            ...
            # Non-linear Part
            def rhs(x, q, entry=None):
                if entry[0] == entry[1]:
                    u = packet.evaluate_at(x, prefactor=True)
                    U = sum(map(lambda xi: abs(xi)**2, u))
                    return kappa * eps**(2*alpha) * U
                else:
                    return zeros_like(x, dtype=complexfloating)

            G = innerproduct.build_matrix(packet, operator=rhs)

            coefficients = packet.get_coefficient_vector()
            coefficients = self._matrix_exponential(F + G, coefficients, dt/eps**2)
